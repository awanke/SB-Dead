...
public class MobileControl extends BaseController {
    private static Logger log = Logger.getLogger(MobileControl.class);
    @Autowired
    ...
    @RequestMapping(value = "/updatePageView")
    public void updatePageView(@RequestParam int articleId, @RequestParam String callback, HttpServletResponse response) {
        // 获取文章对象
        ...
        // 返回最终的访问量
        try {
            response.getWriter().print(callback + "([" + String.valueOf(article.getPageView()) + "])");
        } catch (IOException e) {
            log.error("返回访问量时遇到错误", e);
        }
    }
